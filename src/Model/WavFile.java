...
public class WavFile {
    ...
    public static WavFile openWavFile(String pathFile) throws IOException, WavFileException {
        // Instantiate new Wavfile and store the file reference
        WavFile wavFile = new WavFile();
        wavFile.file = new File(pathFile);

        // Create a new file input stream for reading file data
        wavFile.iStream = new FileInputStream(wavFile.file);
        ...
        if (wavFile.file.length() != chunkSize + 8) {
            throw new WavFileException("Header chunk size (" + chunkSize + ") does not match file size (" + wavFile.file.length() + ")");
        }
