 ...
 */
package JWavRecorder_Vision.Components;

import JWavRecorder_Model.IObserver;
import JWavRecorder_Vision.VFunctions;
...
public class ProgressBar extends JPanel implements IObserver {
    ...
    public ProgressBar(int x, int y, int largura, int altura){
        ...
        //----------------------------------------------------------------------
        lblAmpulheta = new JLabel();
        lblAmpulheta.setName("pnlAmpulheta");
        lblAmpulheta.setSize(
                             (jpbBarraProgresso.getY() - 20 - (lblBarraSuperior.getY() + lblBarraSuperior.getHeight()))*2/3, 
                             jpbBarraProgresso.getY() - 20 - (lblBarraSuperior.getY() + lblBarraSuperior.getHeight())
                            );
        lblAmpulheta.setIcon(VFunctions.getImageRescaledSquare("/JWavRecorder_Figures/ampulheta.png", lblAmpulheta.getSize()));
        lblAmpulheta.setLocation((this.getWidth() - lblAmpulheta.getWidth())/2, lblBarraSuperior.getY() + lblBarraSuperior.getHeight() + 10);
        this.add(lblAmpulheta);
        //----------------------------------------------------------------------
    }
